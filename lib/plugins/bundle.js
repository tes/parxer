...
module.exports = function(handler) {
  ...
  */
  function matchBundles(tagname, attribs, config, state) {
    ...
    _.forEach(state.filterImported('bundle', bundleNames), function(bundleName) {
      ...
      var bundleUrl = bundleBaseUrl + bundle.service + '/' + bundleVersion + '/html/' + bundle.name + '.html';
        ...
        if(inlineAttr) {
          ...
          Core.pushFragment(tagname, bundleAttribs, state, true, function(fragment, next) {
            ...
            if(serverPushAttr) {
              ...
              linkHeader.push('<' + Core.render(directAssetUrl, config.variables) + '>; rel=preload');
              state.setAdditionalHeader('link', linkHeader.join(', '));
            }
            next(null, Core.createAssetTag(bundleType, Core.render(directAssetUrl, config.variables), bundleAttribs));
          });
        }

      } else {
        bundleAttribs[urlAttr] = inlineAttr ? directAssetUrl : bundleUrl;
        Core.pushFragment(tagname, bundleAttribs, state, true, inlineAttr ? Core.renderTextInHandler(handler, config.variables) : handler);
      }
