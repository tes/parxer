...
module.exports = function() {
  ...
  */
  function matchImages(tagname, attribs, config, state) {
    ...
    var resolvedBaseUrl = config.cdn.resolver && config.cdn.resolver(image.service);
    if (resolvedBaseUrl) {
    ...
    Core.pushFragment(tagname, imageAttribs, state, true, function(fragment, next) {
      imageAttribs.src = Core.prepareURL(Core.render(directAssetUrl, config.variables), config.baseURL);
      state.setStatistic('images', imageName, imageAttribs);

      if (clientHintAttr) {
        imageAttribs.rel = imageAttribs.rel || 'preload';
        next(null, Core.getClientHintTag(imageAttribs.src, imageAttribs));
      } else {
