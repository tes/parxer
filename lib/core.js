...
function processDeferredStack(config, state, next) {
    async.mapSeries(state.getDeferredStack(), function(deferred, cb) {
        deferred.fragment.attribs['cx-url'] = render(deferred.fragment.attribs['cx-url'], config.variables);
